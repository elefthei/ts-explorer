...
const viewport:ViewportState&{apply():void;reset():void;zoomAt(delta:number,x:number,y:number):void}={scale:1,x:0,y:0,apply(){$("#svg-holder").style.transform=`translate(${this.x}px,${this.y}px) scale(${this.scale})`;},reset(){this.scale=1;this.x=0;this.y=0;this.apply();const stage=$("#diagram-stage");stage.scrollLeft=0;stage.scrollTop=0;},zoomAt(delta,x,y){zoomViewportAt(this,delta,x,y);this.apply();}};
...
function zoomAtStageCenter(delta:number):void{const rect=diagramStage.getBoundingClientRect();viewport.zoomAt(delta,rect.width/2,rect.height/2);}
